...
int main()
{
	...
	for (int j = 1; j <= J - 1; j++) {
		long double q_e = (1 + func_C(xhb[j + 1]) * tau / h) / 2;
		long double q_f = (1 - func_C(xhb[j + 1]) * tau / h) / 2;
		long double p_f = (1 + func_C(xhb[j]) * tau / h) / 2;
		long double p_e = (1 - func_C(xhb[j]) * tau / h) / 2;

		a2[j] = (tau / 2)* (func_Mu(xhb[j]) / h + func_C(xhb[j]) * (1 - p_e));
		b2[j] = (tau / 2)* (func_Mu(xhb[j + 1]) / h - func_C(xhb[j + 1]) * (1 - q_e));

		long double tmp = h * func_f(x[j]) - func_Mu(xhb[j + 1]) / h - func_Mu(xhb[j]) / h;

		c2[j] = h + (tau / 2)* (tmp - func_C(xhb[j + 1]) * q_e + func_C(xhb[j]) * p_e);

		a[j] = (tau / 2)* (func_Mu(xhb[j]) / h + func_C(xhb[j]) * (1 - p_f));
		b[j] = (tau / 2)* (func_Mu(xhb[j + 1]) / h - func_C(xhb[j + 1]) * (1 - q_f));
		c[j] = h - (tau / 2)* (tmp - func_C(xhb[j + 1]) * q_f + func_C(xhb[j]) * p_f);
	}
