...
int main()
{
	...
	for (int i = 1; i <= 2*N - 1; i++) {
		a[i] = (s[i][колхоз] + 1) / (h*h) - (s[i][колхоз] + 1 + 2*r/(σ*σ) ) / (2 * h);
		b[i] = (s[i][колхоз] + 1) / (h*h) + (s[i][колхоз] + 1 + 2 * r / (σ*σ)) / (2 * h);
		c[i] = 2 / k + (2* s[i][колхоз]+2) / (h * h);
			
		a2[i] = a[i];
		b2[i] = b[i];
		c2[i] = 2 / k - (2 * s[i][колхоз] + 2) / (h * h);
	}
	...
	long double alpha[2*N];//err
	alpha[1] = b[0] / c[0];
	for (int i = 1; i <= 2*N - 1; i++) {
		alpha[i + 1] = b[i] / (c[i] - a[i] * alpha[i]);
	}

	long double betta[2*N];//err
	long double d[2*N];//err
	for (int n = 0; n <= M - 1; n++) {
		d[0] = u[0][n + 1];
		d[2*N] = u[2 * N][n + 1];
		betta[1] = d[0] / c[0];
